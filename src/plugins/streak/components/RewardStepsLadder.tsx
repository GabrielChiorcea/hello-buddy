...
export const RewardStepsLadder: React.FC<RewardStepsLadderProps> = ({
  ...
  return (
    <div
      className={cn(
        'relative',
        manySteps ? 'overflow-x-auto -mx-1 px-1 scrollbar-none' : '',
      )}
    >
      <div className={cn('min-w-full', manySteps && 'min-w-max')}>
        <div className="flex flex-col gap-3">
          ...
          <div className="relative px-2">
            <div className={cn('h-1 rounded-full w-full', colors.track)}>
              <div
                ...
                style={{
                  width: `${Math.min(
                    100,
                    (effectiveCurrent / (sortedSteps[sortedSteps.length - 1]?.stepNumber || 1)) * 100,
                  )}%`,
                }}
              />
            </div>
            <div
              className={cn(
                'absolute left-2 right-2 top-1/2 -translate-y-1/2 flex',
                manySteps ? 'gap-4' : 'justify-between',
              )}
            >
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      manySteps && 'min-w-[60px]',
                    )}
                  >
                    <div
                      className={cn(
                        'w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors',
                        reached ? colors.dotReached : colors.dot,
                      )}
                    >
          ...
          <div
            className={cn(
              'flex text-[10px] sm:text-xs',
              manySteps ? 'gap-4 px-1' : 'justify-between px-1',
            )}
          >
            {sortedSteps.map((step) => {
              const reached = effectiveCurrent >= step.stepNumber;
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    manySteps && 'min-w-[72px]',
                  )}
                >
                  <span
                    className={cn(
                      'leading-snug line-clamp-2 max-w-[90px]',
                      reached ? colors.labelReached : colors.label,
                    )}
                    title={label}
                  >
                    {label}
                  ...
                  <span
                    className={cn(
                      ...
                      reached ? colors.pointsReached : colors.points,
                    )}
                  >
          ...
          {bonusPoints && bonusPoints > 0 && (
            <div className="flex justify-center">
              <span
                className={cn(
                  'mt-1 rounded-full px-2.5 py-0.5 text-[10px] font-medium tabular-nums',
                  completed
